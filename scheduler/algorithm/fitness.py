...
def get_fitness(slots, max_days):
    room_conflict = get_room_conflict(slots)
    faculty_conflict = get_faculty_conflict(slots)
    group_conflict = get_group_conflict(slots)

    faculty_preference_ratio = get_faculty_preference(slots)
    lunch_overlap_ratio = get_lunch_overlap(slots, max_days, [4, 5])
...
def get_room_conflict(slots):
    ...
    for slot in slots:
        ...
        for hour in range(slot.section.duration):
            room_occupied.add((slot.day, slot.time + hour, slot.room))
...
def get_faculty_conflict(slots):
    ...
    for slot in slots:
        ...
        for hour in range(slot.section.duration):
            faculty_assigned.add((slot.day, slot.time + hour, slot.section.faculty))
...
def get_group_conflict(slots):
    ...
    for slot in slots:
        ...
        for hour in range(slot.section.duration):
            group_assigned.add((slot.day, slot.time + hour, slot.section.group))
    return total_groups - len(group_assigned)

def get_faculty_preference(slots):
    max_score = score = 0
    ...
    for slot in slots:
        ...
        for hour in range(slot.section.duration):
            if {'day': slot.day, 'time': slot.time + hour} in slot.section.faculty.slot_choice:
                score += slot.section.faculty.priority

    for faculty in input.faculty_list:
        max_good_slots = min(faculty_section_count[faculty.faculty_id], len(faculty.slot_choice))
        max_score += max_good_slots * faculty.priority

    return score / max_score


def get_lunch_overlap(slots, max_days, lunch_time_slots):
    group_lunch_overlap = defaultdict(int)
    for slot in slots:
        for hour in range(slot.section.duration):
            if slot.time + hour in lunch_time_slots:
                group_lunch_overlap[(slot.section.group, slot.day)] += 1
    day_score = 9
    zero_overlap_score = len(input.group_list) * max_days * day_score
    lunch_overlap = 0
    for value in group_lunch_overlap.values():
        lunch_overlap += value ** 3    # Give worse score for more overlap
    return (zero_overlap_score - lunch_overlap) / zero_overlap_score
