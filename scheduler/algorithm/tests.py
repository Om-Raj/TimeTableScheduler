...
class UtilityFunctionTests(TestCase):
    ...
    def test_save_slots_to_db_updates_all_if_any_exist(self):
        """Should delete old slots and update all with new values if any exist"""

        # Existing slot
        DB_Slot.objects.create(section=self.section, room=self.room1, date_time_slot=self.dts1)

        # New values
        new_room = DB_Room.objects.create(organization=self.org, room_id="R2", capacity=60)
        new_dts = DB_DateTimeSlot.objects.create(organization=self.org, day=2, time=2)
        ...
        updated_slot = Slot(
            section=section_obj,
            room=get_room_object(new_room),
            datetime=DateTimeSlot(day=2, time=2)
        )

        db_slots = save_slots_to_db([updated_slot], self.org.id)

        # Assertions
        self.assertEqual(len(db_slots), 1)
        self.assertEqual(DB_Slot.objects.count(), 1)
        updated_db_slot = DB_Slot.objects.first()
        self.assertEqual(updated_db_slot.room, new_room)
        self.assertEqual(updated_db_slot.date_time_slot, new_dts)

    def test_save_slots_to_db_raises_on_missing_room(self):
        """Should raise ValueError if room does not exist"""
        section_obj = Section(
            id=self.section.id,
            faculty=get_faculty_object(self.faculty1),
            course=get_course_object(self.course1),
        ...
        nonexistent_room = Room(room_id="INVALID", capacity=0)
        slot = Slot(
            section=section_obj,
    ...
    def test_save_slots_to_db_raises_on_missing_datetime(self):
        """Should raise ValueError if datetime slot does not exist"""
        section_obj = Section(
            id=self.section.id,
            faculty=get_faculty_object(self.faculty1),
            course=get_course_object(self.course1),
        ...
        slot = Slot(
            section=section_obj,
            room=get_room_object(self.room1),
            datetime=DateTimeSlot(day=7, time=8)  # Not created in setup
        )
        with self.assertRaises(ValueError):
            save_slots_to_db([slot], self.org.id)